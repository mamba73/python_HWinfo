...
def display_hardware_info():
    """
    Retrieves and displays comprehensive hardware information including CPU, RAM, 
    GPU, Disks (with improved SSD/NVMe detection), and OS details.
    """
    c = wmi.WMI()

    # --- CPU Information ---
    # Using psutil for current frequency and platform for processor name
    ...
    print("\nRAM:")
    total_ram = 0
    # Iterating through physical memory modules for detailed specs
    ...
    # --- GPU Information ---
    print("\nGPU:")
    # GPUtil is the most reliable for detecting NVIDIA specific metrics
    gpus = GPUtil.getGPUs()
    if gpus:
        for gpu in gpus:
    ...
    for i, disk in enumerate(c.Win32_DiskDrive()):
        ...
        if "USBSTOR" in disk.PNPDeviceID or "USB" in interface:
            try:
        ...
        # Determine Drive Type with improved logic for NVMe and SSD
        # WDS100T2B0C models (WD Blue) are often caught via InterfaceType or Model substrings
        if "USB" in interface or "USB" in model:
            drive_type = f"USB External ({vendor})"
        elif "NVME" in interface or "NVME" in model or "WDS100T2B0C" in model:
            drive_type = "NVMe SSD"
        elif "SSD" in model or "S400S37" in model: # S400S37 is Kingston A400 SSD series
            drive_type = "SSD"
        else:
            drive_type = "HDD"
        ...
        for partition in disk.references("Win32_DiskDriveToDiskPartition"):
            for logical_disk in partition.Dependent.references("Win32_LogicalDiskToPartition"):
                ...
                try:
                    # Trailing backslash is required for psutil.disk_usage on Windows
